...
OUT_PATH = REPO_ROOT / "test_outputs" / "selected_nonlinear_unified_heatmap.png"
...
def _load_nonlinear_score(run_dir: Path) -> float:
    path = run_dir / "probe_nonlinear" / "nonlinear_probe_results.json"
    if not path.exists():
        return float("nan")
    try:
...
def _choose_best(config: str, method: str) -> Optional[float]:
    best = None
...
def plot() -> None:
    ...
    ax.set_title("Nonlinear Probe Results", fontsize=14, pad=24)

    plt.tight_layout()
    OUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(OUT_PATH, dpi=180, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved nonlinear heatmap to: {OUT_PATH}")
